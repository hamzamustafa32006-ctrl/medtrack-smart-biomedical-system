...
- **Pages**:
    - **Maintenance Records Page**: Professional maintenance tracking with advanced filtering system including multi-select filters (status, type), cost range, date range, server-side pagination (20 records/page), sortable columns (equipment name, type, date, status, cost), active filter chips with quick removal, search functionality, interactive table, add maintenance dialog, and comprehensive details sheet. Uses custom `useMaintenance` hook with TanStack Query for efficient data fetching and cache management.
...
- **Maintenance Record Management API**: Comprehensive CRUD operations with advanced filtering system (`/api/maintenance-records/details`) supporting multi-value filters (status, maintenance type), text search (equipment name, serial, technician), cost range filtering, date range filtering, server-side sorting (equipment name, type, date, status, cost), and pagination with metadata (page, total, totalPages). Uses Drizzle ORM's `and()` and `inArray()` for efficient multi-condition queries. Includes creation with automatic equipment status updates ("Under Maintenance"), and completion with automated workflows (alert resolution, status update to "Active", next scheduled date calculation, verification support).
