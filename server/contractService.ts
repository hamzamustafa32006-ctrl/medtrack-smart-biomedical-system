  ...
  try {
    ...
    for (const contract of contractList) {
      try {
        ...
        // Update status to Expired if past end date and currently Active
        if (daysUntilExpiry < 0 && contract.status === "Active") {
          ...
          // Create critical alert for expired contract (unless auto-renew is enabled)
          ...
          // Create warning alert for contract expiring soon
          try {
